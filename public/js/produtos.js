...
async function carregarTipos() {
  ...
  select.innerHTML = '<option value="">—</option>' + tipos.map(t => `<option value="${t.id_tipo}">${t.nome}</option>`).join('');
  return tipos;
}
...
async function carregarProdutos() {
  ...
  for (const p of produtos) {
    ...
    linha.innerHTML = `
      <td>${p.referencia}</td><td>${p.descricao}</td><td>${p.tipo || '—'}</td><td>${p.genero || '—'}</td>
...
async function alternarVariacoes(idProduto, botao) {
  ...
  if (aberta) { linha.style.display = 'none'; botao.textContent = 'Ver variações'; return; }
...
async function renderizarVariacoes(idProduto) {
  ...
  const linhasTabela = variacoes.map(v => `
    <tr>
      <td>${v.tamanho}</td><td>${v.cor}</td><td>${v.codigo_barras || '—'}</td>
      <td>${v.quantidade_atual <= v.estoque_minimo ? `<span class="badge-baixo">${v.quantidade_atual}</span>` : `<span class="badge-ok">${v.quantidade_atual}</span>`}</td>
    </tr>
  ...
  document.getElementById(`formVariacao-${idProduto}`).addEventListener('submit', async (e) => {
    ...
    try {
      await apiFetch(`/api/produtos/${idProduto}/variacoes`, {
        method: 'POST',
        body: JSON.stringify({
          tamanho: form.tamanho.value, cor: form.cor.value,
...
document.getElementById('formTipo').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    msg.textContent = 'Tipo adicionado.'; msg.className = 'msg sucesso';
    await carregarTipos();
  } catch (erro) { msg.textContent = erro.message; msg.className = 'msg erro'; }
});
...
document.getElementById('formProduto').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    msg.textContent = 'Produto cadastrado com sucesso.'; msg.className = 'msg sucesso';
    await carregarProdutos();
  } catch (erro) { msg.textContent = erro.message; msg.className = 'msg erro'; }
});

(async function iniciar() { await carregarTipos(); await carregarProdutos(); })();
